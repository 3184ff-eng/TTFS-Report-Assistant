...
test("long official fields create appendix continuation pages", () => {
  ...
  assert.ok(appendices[0].sections.some((section) => section.section === "Officer's observations Continued"));
});

test("long owner occupier and report fields are combined into professional appendix pages", () => {
  const form = completeForm({
    ownerOccupier:
      "Owner: Phoenix Holdings Limited. Occupiers: Alpha Pharmacy Limited, O'Meara Food Mart Limited, and Steven Baptiste.",
    typeOfProperty: "Single-storey commercial property with mixed masonry and timber construction. ".repeat(8)
  });

  const appendices = logic.buildAppendices(form);
  const continuedSections = appendices.flatMap((appendix) => appendix.sections.map((section) => section.section));
